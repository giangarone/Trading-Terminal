...
- **Watchlist toggle in the Symbol Selector** — Every row in the redesigned Symbol Selector has an add/remove-from-watchlist button (a **+** that becomes a **✓** when the symbol is watchlisted). Adding or removing there updates the Watchlist panel instantly, and the toggle always reflects the symbol's current watchlist status.
