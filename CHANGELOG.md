...
* **Cross-venue trading** — The chart you analyse on and the exchange you trade through are now two separate choices. A venue selector beside the symbol sets them independently: watch Binance candles, execute on BloFin, and never leave your chart to place or manage a position. Orders, take profits, stop losses, working closes, quantities, position and P&L all draw on the analysis chart as before; each execution object carries a small badge naming the venue it belongs to, so it is never ambiguous whose book an order is in. Order placement, amendment and cancellation always go to the selected execution venue regardless of which exchange is supplying the candles. The execution venue is the account in the topbar — picking one in either place moves the other — and any venue without a connected account is listed but points you at Broker Connections instead.
...
* **Dual prices on chart levels** — When the two venues have drifted far enough apart for the difference to matter, order lines state both numbers: the level on the chart, and underneath it the price the order actually works at on its venue. Right-axis tags do the same. When the venues track closely, or when you are charting and executing on the same exchange, the chart looks exactly as it always has.
